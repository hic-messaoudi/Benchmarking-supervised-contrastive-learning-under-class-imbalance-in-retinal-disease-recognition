"""
Supervised Contrastive Learning (SCL) Training Script
Trains a model using SupCon loss combined with classification loss.
"""
...
class SupConLoss(nn.Module):
    ...
    def forward(self, features, labels=None):
        ...
        # Tile mask and remove self-contrast
    ...
    def fit(self, train_loader, val_loader, fold_num, epochs=EPOCHS, lr=LEARNING_RATE):
        ...
        for epoch in range(epochs):
            ...
            if val_metrics['auprc'] > best_auprc:
                ...
                torch.save(self.model, os.path.join(CURRENT_DIR, f'SCL_foldp_{fold_num}.pt'))
    ...
    def fit(self, train_loader, val_loader, fold_num, epochs=EPOCHS, lr=LEARNING_RATE):
        ...
        for epoch in range(epochs):
            ...
            if val_metrics['auroc'] > best_auroc:
                ...
                no_improve = 0
                print(f"  New best AUROC: {best_auroc:.4f}")
                torch.save(self.model, os.path.join(CURRENT_DIR, f'SCLfS_fold_{fold_num}.pt'))
            else:
                no_improve += 1
                if no_improve >= self.patience:
                    print(f"Early stopping at epoch {epoch+1}")
