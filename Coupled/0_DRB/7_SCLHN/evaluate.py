"""
Evaluation script for SCLHN (Coupled) method.
"""
import torch
...
class Classifier(nn.Module):
    def __init__(self, backbone_path):
        super().__init__()
        encoder = torch.load(backbone_path, map_location='cuda:0', weights_only=False)
...
def extract_contrastive_features(model, data_loader, device):
    model.train()
    features, labels = [], []
    with torch.no_grad():
        for images, labs in data_loader:
            out, _ = model(images.to(device))
...
def main():
    ...
    # =========================================================================
    print_section_header("Part 1: Contrastive Model (SCLHN_fold_*.pt)")
    
    contrastive_models, cv_metrics = [], []
    cv_preds_list, cv_labels_list = [], []
    for fold in range(1, 6):
        model_path = os.path.join(current_dir, f'SCLHN_fold_{fold}.pt')
        if not os.path.exists(model_path): continue
        model = torch.load(model_path, map_location=device, weights_only=False)
        contrastive_models.append(model)
    ...
    if cv_metrics:
        print("\nCross-Validation (Classification Head):")
        ...
        print_stat_comparison_cv(cv_metrics, supervised_cv, 'SCLHN', pooled_preds, pooled_labels, sup_cv_preds, sup_cv_labels)
        save_results_to_csv(cv_metrics, cv_stats, os.path.join(current_dir, 'results_contrastive.csv'))
    
    if contrastive_models:
        print("\n  Generating t-SNE visualizations...")
        features, labels_np = extract_contrastive_features(contrastive_models[0], test_loader, device)
        plot_tsne_2d(features, labels_np, os.path.join(current_dir, 'tsne_2d.png'), "SCLHN t-SNE 2D")
        plot_tsne_sphere(features, labels_np, os.path.join(current_dir, 'tsne_sphere.png'), "SCLHN t-SNE Sphere")
        
        print_section_header("Contrastive Model - Ensemble Test")
        pred_avg, labels = evaluate_ensemble(contrastive_models, test_loader, device)
        ens = calculate_fold_metrics(pred_avg, labels)
        print_all_metrics(ens, "  ")
        print_ensemble_comparison(pred_avg.numpy(), labels.numpy(), sup_test_preds, sup_test_labels, ens['auroc'], supervised_test_auroc, 'SCLHN')
    
    # =========================================================================
    # PART 2: Finetuned Classifier
    # =========================================================================
    print_section_header("Part 2: Finetuned Classifier (SCLHNfS_fold_*.pt)")
    
    finetuned_models, cv_metrics_ft = [], []
    cv_preds_ft, cv_labels_ft = [], []
    for fold in range(1, 6):
        # Try both naming conventions
        model_path = os.path.join(current_dir, f'SCLHNfS_fold_{fold}.pt')
        if not os.path.exists(model_path):
            model_path = os.path.join(current_dir, f'SCLHNS_fold_{fold}.pt')
        if not os.path.exists(model_path): continue
        model = torch.load(model_path, map_location=device, weights_only=False)
        model.eval()
        ...
        val_loader = create_validation_dataloader(val_data, val_labels, batch_size=batch_size, num_workers=8)
        preds, labs = evaluate_fold(model, val_loader, device)
        cv_metrics_ft.append(calculate_fold_metrics(preds, labs))
        cv_preds_ft.append(preds)
        cv_labels_ft.append(labs)
    
    if cv_metrics_ft:
        print("\nCross-Validation (Finetuned):")
        cv_stats_ft = compute_statistics(cv_metrics_ft)
        print(format_results_table(cv_metrics_ft, cv_stats_ft))
        
        pooled_preds_ft = torch.cat(cv_preds_ft, dim=0).numpy()
        pooled_labels_ft = torch.cat(cv_labels_ft, dim=0).numpy()
        print_stat_comparison_cv(cv_metrics_ft, supervised_cv, 'SCLHNS', pooled_preds_ft, pooled_labels_ft, sup_cv_preds, sup_cv_labels)
        save_results_to_csv(cv_metrics_ft, cv_stats_ft, os.path.join(current_dir, 'results_finetuned.csv'))
    ...
    if finetuned_models:
        ...
        ens = calculate_fold_metrics(pred_avg, labels)
        print_all_metrics(ens, "  ")
        print_ensemble_comparison(pred_avg.numpy(), labels.numpy(), sup_test_preds, sup_test_labels, ens['auroc'], supervised_test_auroc, 'SCLHNS')
