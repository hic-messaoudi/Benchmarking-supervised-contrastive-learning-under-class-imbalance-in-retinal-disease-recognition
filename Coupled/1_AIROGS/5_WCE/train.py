"""
Weighted Cross-Entropy (WCE) Training Script
Trains an EfficientNet model using weighted BCE loss to handle class imbalance.
"""
...
class WeightedBCELoss(torch.nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        loss = -(weights * (targets * torch.log(inputs) + 
...
class Trainer:
    """Training class for WCE."""
    
    def __init__(self, pos_weight, device=DEVICE):
        self.device = device
    ...
    def fit(self, train_loader, val_loader, fold_num, epochs=EPOCHS, lr=LEARNING_RATE):
        ...
        for epoch in range(epochs):
            ...
            if val_metrics['auroc'] > best_auroc:
                ...
                torch.save(self.model, os.path.join(CURRENT_DIR, f'WCE_fold_{fold_num}.pt'))
            
            if val_metrics['auprc'] > best_auprc:
                best_auprc = val_metrics['auprc']
                improved = True
                print(f"  New best AUPRC: {best_auprc:.4f}")
                torch.save(self.model, os.path.join(CURRENT_DIR, f'WCE_foldp_{fold_num}.pt'))
...
def main():
    ...
    for fold in range(5):
        ...
        train_dataset, val_dataset, train_labels = create_fold_datasets(fold, all_data)
        
        # Calculate class weights
        neg_count = np.sum(train_labels == 0)
        pos_count = np.sum(train_labels == 1)
        pos_weight = neg_count / max(pos_count, 1)
        print(f"Class weight: {pos_weight:.2f} (neg: {neg_count}, pos: {pos_count})")
        ...
        trainer = Trainer(pos_weight=pos_weight)
