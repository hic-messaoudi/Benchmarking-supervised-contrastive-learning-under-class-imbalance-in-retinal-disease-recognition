"""
SCLHN (Supervised Contrastive Learning with Hybrid Networks) Training Script
Uses a decaying alpha to balance contrastive and classification loss.
"""
...
class Trainer:
    """Training class for SCLHN with decaying alpha."""
    ...
    def train_epoch(self, train_loader, optimizer, alpha):
        """Train for one epoch with given alpha weight."""
        self.model.train()
        total_con_loss = 0
        total_cls_loss = 0
        ...
        for img1, img2, labels in train_loader:
            ...
            # Hybrid loss: starts with more contrastive, shifts to classification
            loss = alpha * con_loss + (1 - alpha) * cls_loss
    ...
    def fit(self, train_loader, val_loader, fold_num, epochs=EPOCHS, lr=LEARNING_RATE):
        """Train the model with decaying alpha."""
        ...
        for epoch in range(epochs):
            ...
            # Decaying alpha: starts at 1 (pure contrastive), ends at 0 (pure classification)
            alpha = 1 - (epoch / epochs)
            
            train_metrics = self.train_epoch(train_loader, optimizer, alpha)
            duration = time.time() - start
            
            print(f"Epoch {epoch+1}/{epochs} ({duration:.0f}s) [α={alpha:.3f}] - "
            ...
            if val_metrics['auroc'] > best_auroc:
                ...
                torch.save(self.model, os.path.join(CURRENT_DIR, f'SCLHN_fold_{fold_num}.pt'))
            
            if val_metrics['auprc'] > best_auprc:
                best_auprc = val_metrics['auprc']
                improved = True
                print(f"  New best AUPRC: {best_auprc:.4f}")
                torch.save(self.model, os.path.join(CURRENT_DIR, f'SCLHN_foldp_{fold_num}.pt'))
...
class Finetuner:
    ...
    def __init__(self, fold_num, device=DEVICE):
        ...
        backbone_path = os.path.join(CURRENT_DIR, f'SCLHN_fold_{fold_num}.pt')
    ...
    def fit(self, train_loader, val_loader, fold_num, epochs=EPOCHS, lr=LEARNING_RATE):
        ...
        for epoch in range(epochs):
            ...
            if val_metrics['auroc'] > best_auroc:
                ...
                no_improve = 0
                print(f"  New best AUROC: {best_auroc:.4f}")
                torch.save(self.model, os.path.join(CURRENT_DIR, f'SCLHNfS_fold_{fold_num}.pt'))
            else:
                no_improve += 1
                if no_improve >= self.patience:
                    print(f"Early stopping at epoch {epoch+1}")
