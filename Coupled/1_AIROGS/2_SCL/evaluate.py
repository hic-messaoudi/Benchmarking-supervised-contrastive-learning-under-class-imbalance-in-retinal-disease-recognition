"""
Evaluation script for SCL (Coupled) method.
"""
import torch
...
def main():
    ...
    # =========================================================================
    print_section_header("Part 1: Contrastive Model (SCL_fold_*.pt)")
    
    contrastive_models, cv_metrics = [], []
    cv_preds_list, cv_labels_list = [], []
    for fold in range(1, 6):
        model_path = os.path.join(current_dir, f'SCL_fold_{fold}.pt')
    ...
    if cv_metrics:
        ...
        print_stat_comparison_cv(cv_metrics, supervised_cv, 'SCL', pooled_preds, pooled_labels, sup_cv_preds, sup_cv_labels)
    ...
    if contrastive_models:
        ...
        print_ensemble_comparison(pred_avg.numpy(), labels.numpy(), sup_test_preds, sup_test_labels, ens['auroc'], supervised_test_auroc, 'SCL')
    
    # =========================================================================
    # PART 2: Finetuned Classifier
    # =========================================================================
    print_section_header("Part 2: Finetuned Classifier (SCLfS_fold_*.pt)")
    
    finetuned_models, cv_metrics_ft = [], []
    cv_preds_ft, cv_labels_ft = [], []
    for fold in range(1, 6):
        model_path = os.path.join(current_dir, f'SCLfS_fold_{fold}.pt')
        if not os.path.exists(model_path): continue
        model = torch.load(model_path, map_location=device, weights_only=False)
        model.eval()
    ...
    if cv_metrics_ft:
        ...
        print_stat_comparison_cv(cv_metrics_ft, supervised_cv, 'SCLfS', pooled_preds_ft, pooled_labels_ft, sup_cv_preds, sup_cv_labels)
    ...
    if finetuned_models:
        ...
        ens = calculate_fold_metrics(pred_avg, labels)
        print_all_metrics(ens, "  ")
        print_ensemble_comparison(pred_avg.numpy(), labels.numpy(), sup_test_preds, sup_test_labels, ens['auroc'], supervised_test_auroc, 'SCLfS')
