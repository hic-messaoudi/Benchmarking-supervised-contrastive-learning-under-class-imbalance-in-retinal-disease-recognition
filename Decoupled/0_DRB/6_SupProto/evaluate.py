"""
Evaluation script for SupProto (Decoupled) method.
"""
import torch
...
def main():
    ...
    # =========================================================================
    print_section_header("Part 1: Contrastive Encoder (SupProto_fold_*.pt)")
    
    encoder_path = os.path.join(current_dir, 'SupProto_fold_1.pt')
    ...
    # =========================================================================
    print_section_header("Part 2: Finetuned Classifier (SupProto_classifier_fold_*.pt)")
    
    finetuned_models, cv_metrics = [], []
    cv_preds_list, cv_labels_list = [], []
    for fold in range(1, 6):
        model_path = os.path.join(current_dir, f'SupProto_classifier_fold_{fold}.pt')
    ...
    if cv_metrics:
        ...
        print_stat_comparison_cv(cv_metrics, supervised_cv, 'SupProtoS', pooled_preds, pooled_labels, sup_cv_preds, sup_cv_labels)
    ...
    if finetuned_models:
        ...
        print_ensemble_comparison(pred_avg.numpy(), labels.numpy(), sup_test_preds, sup_test_labels, ens['auroc'], supervised_test_auroc, 'SupProtoS')
