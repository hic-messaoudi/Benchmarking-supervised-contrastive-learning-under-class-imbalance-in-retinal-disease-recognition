"""
Decoupled ProCo (Probabilistic Contrastive) Training Script
Trains only the contrastive loss with von Mises-Fisher distribution.
"""
...
class Trainer:
    """Training class for Decoupled ProCo."""
    
    def __init__(self, device=DEVICE):
        self.device = device
        self.patience = PATIENCE
        self.contrastive_criterion = ProCoLoss(device=device)
    ...
    def train_epoch(self, train_loader, optimizer):
        ...
        return {'proco': total_loss / len(train_loader)}

    @torch.no_grad()
    def validate(self, val_loader):
        """Validate using ProCo loss."""
        self.model.eval()
        total_loss = 0
        ...
        return {'proco': total_loss / len(val_loader)}
    ...
    def fit(self, train_loader, val_loader, fold_num, epochs=EPOCHS, lr=LEARNING_RATE):
        ...
        for epoch in range(epochs):
            ...
            print(f"Epoch {epoch+1}/{epochs} ({duration:.0f}s) - "
                  f"Train Loss: {train_metrics['proco']:.4f}")
            
            val_metrics = self.validate(val_loader)
            print(f"  Val Loss: {val_metrics['proco']:.4f}")
            
            if val_metrics['proco'] < best_loss:
                best_loss = val_metrics['proco']
                no_improve = 0
...
def main():
    ...
    print("\n" + "="*60)
    print("STAGE 1: Contrastive Learning (ProCo)")
    print("="*60)
    ...
    def fit(self, train_loader, val_loader, fold_num, epochs=EPOCHS, lr=LEARNING_RATE):
        ...
        for epoch in range(epochs):
            ...
            if val_metrics['auroc'] > best_auroc:
                ...
                torch.save(self.model, os.path.join(CURRENT_DIR, f'ProCo_classifier_fold_{fold_num}.pt'))
            else:
                no_improve += 1
                if no_improve >= self.patience:
                    print(f"Early stopping at epoch {epoch+1}")
