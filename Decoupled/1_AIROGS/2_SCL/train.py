"""
Decoupled SCL (Supervised Contrastive Learning) Training Script
Trains only the contrastive loss without classification head.
"""
...
class ContrastiveDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Always return two views for contrastive learning
...
class Trainer:
    """Training class for Decoupled SCL."""
    ...
    def train_epoch(self, train_loader, optimizer):
        ...
        return {'contrastive': total_loss / len(train_loader)}

    @torch.no_grad()
    def validate(self, val_loader):
        """Validate using contrastive loss."""
        self.model.eval()
        total_loss = 0
        ...
        return {'contrastive': total_loss / len(val_loader)}
    ...
    def fit(self, train_loader, val_loader, fold_num, epochs=EPOCHS, lr=LEARNING_RATE):
        ...
        for epoch in range(epochs):
            ...
            print(f"Epoch {epoch+1}/{epochs} ({duration:.0f}s) - "
                  f"Train Loss: {train_metrics['contrastive']:.4f}")
            
            val_metrics = self.validate(val_loader)
            print(f"  Val Loss: {val_metrics['contrastive']:.4f}")
            
            if val_metrics['contrastive'] < best_loss:
                best_loss = val_metrics['contrastive']
                no_improve = 0
                print(f"  New best loss: {best_loss:.4f}")
                torch.save(self.model, os.path.join(CURRENT_DIR, f'SCL_fold_{fold_num}.pt'))
            else:
                no_improve += 1
...
class ClassificationDataset(torch.utils.data.Dataset):
    """Dataset for classification (single image, no dual augmentation)."""
...
class Finetuner:
    ...
    def __init__(self, fold_num, device=DEVICE):
        ...
        backbone_path = os.path.join(CURRENT_DIR, f'SCL_fold_{fold_num}.pt')
    ...
    @torch.no_grad()
    def validate(self, val_loader):
        """Validate using classification metrics."""
        self.model.eval()
        ...
        auroc = binary_auroc(all_preds, all_labels).item()
    ...
    def fit(self, train_loader, val_loader, fold_num, epochs=EPOCHS, lr=LEARNING_RATE):
        ...
        for epoch in range(epochs):
            ...
            if val_metrics['auroc'] > best_auroc:
                ...
                torch.save(self.model, os.path.join(CURRENT_DIR, f'SCL_classifier_fold_{fold_num}.pt'))
            else:
                no_improve += 1
                if no_improve >= self.patience:
                    print(f"Early stopping at epoch {epoch+1}")
